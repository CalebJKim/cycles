...
import time
from db import add_summary_data
import json
...
def cycle(cycle_num, llm, input, persona_prompts, prev_summary):
    ...
    for p in persona_prompts:
        ...
        p_response = llm.invoke(new_input)
        print(p_response)
    #create summary
    summary_response = llm.invoke(
        "Summarize the following sentiments and opinions from different people into 1-2 sentences. Then, give me a score from 0 to 100, with 0 being complete negativity while 100 being complete positivity. Any sentiments that are negative or opinions that express a taking of offense (for example, racism and sexism) should be given a drastically larger weighting, since we want to avoid all controversey. Format your response as summary~score " + "; ".join(p_response)
    )
    summary_responses = summary_response.split("~")
    summary = summary_responses[0]
    score = summary_responses[1]
...
async def run_cycles(ctx: Context, req: Request) -> Response:
    ctx.logger.info(f"Received POST request with question {req.text} and filepath {req.file}")
    #develop personas
    llm = GeminiLLM()
    ...
    for x in range(CYCLES):
        summ, score = cycle(x, llm, [req.text, req.file], persona_prompts, summaries[-1])
        summaries.append(summ)
    
    add_summary_data((CYCLES, summaries[-1], score))
    return summaries
